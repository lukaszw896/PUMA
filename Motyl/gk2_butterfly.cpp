...
//Compute dodecahedronMtx and mirrorMtx
{
	...
	for (int i = 0; i < 12; i++) {
		XMVECTOR det;
		m_mirrorMtx[i] = XMMatrixInverse(&det, m_dodecahedronMtx[i]) * XMMatrixScaling(1, 1, -1) * m_dodecahedronMtx[i];
	}
}
...
//Compute the matrices for butterfly wings. Position on the strip is determined based on time
{
	...
	/*MoebiusStripPos(float t, float s)

	MoebiusStripDt(float t, float s)
	...
	*/
	...
	XMVECTOR pn = XMVector3Normalize(XMVector3Cross(ps, pt));
	XMVECTOR p = XMLoadFloat3(&MoebiusStripPos(t, 0));
	p.m128_f32[3] = 1;




 	XMMATRIX  boxMatrix = XMMATRIX(ps, pt, pn, p);

	m_wingMtx[0] = m_wingMtx[1] = XMMatrixTranslation(1.0f, 0.0f, 0.0f) *
		XMMatrixScaling(0.5f * WING_H, 0.5f * WING_W, 1.0f) *
		XMMatrixRotationY(XM_PIDIV2);
	m_wingMtx[0] *= XMMatrixRotationY(a) * boxMatrix;
	m_wingMtx[1] *= XMMatrixRotationY(-a) * boxMatrix;
...
//Draw dodecahedron. If color is true, use render faces with coresponding colors. Otherwise render using white color
{
	...
	for (auto i = 0; i < 12; ++i)
	{
		...
		//else SetSurfaceColor(XMFLOAT4(1.0,1.0,1.0,1.0));
...
//Draw the butterfly
{
	ID3D11Buffer* buff = m_vbWing.get();
	m_context->IASetVertexBuffers(0, 1, &buff, &VB_STRIDE, &VB_OFFSET);
...
void Butterfly::Render()
{
	...
	DrawDodecahedron(true); DrawButterfly();
